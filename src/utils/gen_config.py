...
def get_llm(**kwargs) -> LLM:
    ...
    if provider == "anthropic":
        ...
        try:
            ...
            raise Exception(f"gen_config: Failed to get {provider} LLM") from e
    elif kwargs["provider"] == "vllm":
        # 使用本地vllm模型，通过OpenAI兼容的API接口
        try:
            # 从配置文件或环境变量中获取vllm服务器地址，默认为本地地址
            vllm_base_url = cfg.get("VLLM_BASE_URL", "http://localhost:8000/v1")
            # vllm不需要真实的API key，但OpenAI客户端需要一个非空字符串
            vllm_api_key = cfg.get("VLLM_API_KEY", "EMPTY")
            
            logger.info(f"使用本地vllm模型: {kwargs['model']}, 服务地址: {vllm_base_url}")
    ...
    # 对于vllm provider，跳过测试连接（因为本地服务器可能还未完全启动）
    if provider != "vllm":
        try:
        ...
        logger.info("跳过vllm连接测试，将在实际调用时验证连接")
