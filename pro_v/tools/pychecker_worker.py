...
def get_ray_pychecker_worker_cls():
    """
    ...
    num_cpus_per_worker = float(os.getenv("PYCHECKER_WORKER_CPUS", "0.4"))
    ...
    class _RayPyCheckerWorker:
        ...
        async def simulate_dut_seq_ray(self, output_dir: str) -> Tuple[int, str, str, float]:
            ...
            sim_template_dir = os.path.join(os.path.abspath("/home/lah003/workspace/verl_efficient/pettingllms/multi_agent_env/pychecker_rl/sim_seq"))
            ...
            for file_name in files_to_copy:
                ...
                subprocess.run(f"cp {src} {work_dir}/", shell=True)
            
            subprocess.run(f"cp {dut_path} {work_dir}/top_module.v", shell=True)
            subprocess.run(f"cp {test_path} {work_dir}/testbench.json", shell=True)
            
            # Execute compilation and simulation (CPU-intensive operation)
            # With many workers running simultaneously, limit jobs per make to avoid CPU oversubscription
            # Each worker has limited CPU (e.g., 0.4), so use serial make by default
            make_jobs = os.getenv("PYCHECKER_MAKE_JOBS", "1")
            harness_generator_path = os.path.join(sim_template_dir, "harness-generator.py")
            cmd = f"cd {work_dir} && python {harness_generator_path} && make -j{make_jobs}"
            try:
        ...
        async def simulate_dut_cmb_ray(self, output_dir: str) -> Tuple[int, str, str, float]:
            ...
            sim_template_dir = os.path.join(os.path.abspath("/home/lah003/workspace/verl_efficient/pettingllms/multi_agent_env/pychecker_rl/sim_cmb"))
